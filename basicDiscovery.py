...
from deploy_package.greengrasssdk.stream_manager import data
...
loopCount = 0
data_csv = pd.read_csv(data_path.format(0))
for co2_val in data_csv["vehicle_CO2"]:
    print("co2 val: ", co2_val)
    if args.mode == 'both' or args.mode == 'publish':
        message = {}
        message['message'] = args.message
        message['co2'] = co2_val
        messageJson = json.dumps(message)
        myAWSIoTMQTTClient.publish(topic+"/trigger", messageJson, 0)
        if args.mode == 'publish':
            print('Published topic %s: %s\n' % (topic+"/trigger", messageJson))
        loopCount += 1
    time.sleep(1)
